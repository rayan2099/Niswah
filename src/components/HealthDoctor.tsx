...
ما اقتراحاتك؟`;
    ...
    try {
      ...
      setMessages([
        { role: 'user', text: `أعاني من: ${symptomsText}${userNotes ? `\n\nملاحظات: ${userNotes}` : ''}` },
        { role: 'ai', text: `عذراً، لم أتمكن من الاتصال. (v4.0-FINAL | Error: ${serverError})` },
      ]);
    }
  };
...
- Avoid complex medical jargon.`;
      ...
      const serverError = typeof rawError === 'object' ? JSON.stringify(rawError) : String(rawError);
      setIsTyping(false);
      setMessages(prev => [...prev, { role: 'ai', text: `عذراً، حدث خطأ في الاتصال. (v4.0-FINAL | Error: ${serverError})` }]);
    }
  };
