...
export const Today = ({ 
  ...
  const renderDailyWellnessCard = () => {
    if (!isInTahara || isPregnant) return null;
    ...
    const contextLabel = isRTL ? 'متابعة الحالة النفسية' : 'Mental state check-in';
    const helperText = isRTL
      ? 'دوّني مزاجك، طاقتك، نومك وأي أعراض خفيفة لتفهمي نمطك بهدوء.'
    ...
    return (
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-sm overflow-hidden rounded-[34px] border border-emerald-100/80 bg-white shadow-2xl shadow-emerald-950/5"
        dir={isRTL ? 'rtl' : 'ltr'}
      >
        <div className="relative bg-gradient-to-br from-emerald-50 via-white to-rose-50 p-5 space-y-5">
          ...
          <div className="relative grid grid-cols-3 gap-2">
            {[
              { label: t('mood'), value: todayEntry ? moodLabel : '—', className: 'bg-white/90 text-emerald-800 border-emerald-100' },
        ...
        {todayEntry ? (
          ...
          <div className="p-5">
            <button
              onClick={openMentalLog}
              className="w-full rounded-3xl border border-emerald-200 bg-emerald-50 px-4 py-5 text-sm font-bold text-emerald-800 shadow-inner shadow-white active:scale-[0.99] transition-transform"
            >
              {isRTL ? 'سجلي حالتك النفسية الآن' : 'Log your mental state now'}
            </button>
          ...
          <section className="flex flex-col items-center py-6 space-y-6">
            {isPregnant ? (
              ...
              <>
                ...
                {isInTahara && renderDailyWellnessCard()}
